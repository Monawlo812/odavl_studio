...
const list = readdirSync(".")
  .filter(isJS)
  .filter((f) => !protectedRx.test(f));

const allow = new Set(
  (existsSync(allowlistPath)
    ? readFileSync(allowlistPath, "utf-8").split(/\r?\n/)
    : []
  ).filter(Boolean),
);
const selected = [];
...
const out = {
  ...
  score: files.length ? Math.floor(totalSites / files.length) : 0,
  note: "preview-only (no test execution)",
};
