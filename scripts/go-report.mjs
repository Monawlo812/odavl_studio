...
function sh(cmd, args = [], timeoutMs = 15000) {
  try {
    ...
    return { ok: false, code: 1, out: "", err: String(e && (e.message || e)) };
  }
}
...
const apps = fs.existsSync("apps")
  ? fs.readdirSync("apps").filter((x) => x !== "node_modules")
  : [];
const packages = fs.existsSync("packages")
  ? fs.readdirSync("packages").filter((x) => x !== "node_modules")
  : [];
...
function pkgMeta(p) {
  ...
  return {
    ...
    odavlDeps: Object.keys({
      ...(j.dependencies || {}),
      ...(j.devDependencies || {}),
    }).filter((k) => k.startsWith("@odavl/")),
  };
}
const allPkgs = [];
...
let statusJson = null;
try {
  statusJson = JSON.parse(cliStatus.out);
} catch {
  statusJson = { ok: false, note: "status failed" };
}
fs.writeFileSync(
  path.join(rptDir, "status.json"),
...
const extMeta = extPkg
  ? {
      ...
      name: extPkg.name,
      commands: (extPkg.contributes && extPkg.contributes.commands) || [],
      activation: extPkg.activationEvents || [],
      statusbar: !!(extPkg.contributes && extPkg.contributes.viewsContainers),
    }
  : { note: "not found" };
...
try {
  orchApi = sh("node", [
    "-e",
    "import('./apps/orchestrator/dist/index.js').then(m=>console.log(!!m.runWorkflow))",
  ]);
} catch {
  orchApi = { ok: false, note: "not built" };
}
...
// 8. Policy & Gates
const policyText =
  readTextIf(".odavl.policy.yml") || readTextIf(".odavl/policy.yml");
...
const ghRuns = sh("gh", [
  ...
  "10",
  "--json",
  "status,conclusion,workflowName,htmlUrl",
]);

// 10. Python Pack
const pyPack = fs.existsSync("packages/packs/python")
  ? { present: true }
  : { present: false };
...
const snapshot = {
  ts: now,
  git: {
    branch: gitBranch.out.trim(),
    tag: gitTag.out.trim(),
    sha: gitSha.out.trim(),
    dirty: !!gitStatus.out.trim(),
  },
  node: nodeV.out.trim(),
  pnpm: pnpmV.out.trim(),
  workspaces,
  ...
  ci: {
    workflows,
    shadow: shadowExists,
    prs: ghPrs.ok ? JSON.parse(ghPrs.out) : [],
    runs: ghRuns.ok ? JSON.parse(ghRuns.out) : [],
  },
...
fs.writeFileSync(
  path.join(rptDir, "snapshot.json"),
...
const md = [
  ...
  ...allPkgs.map(
    (p) =>
      `- ${p.name}@${p.version} (private:${!!p.private}) scripts:${p.scripts.length} odavlDeps:${p.odavlDeps.length}`,
  ),
  "",
  "## CLI Surface",
...
console.log(
  "GO-REPORT: written reports/go-report/OPERATIONS-REPORT.md and snapshot.json",
);
