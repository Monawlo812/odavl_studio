...
const cwd = process.cwd(), now = new Date().toISOString();
const rptDir = path.join(cwd, 'reports');
fs.mkdirSync(rptDir, { recursive: true });
const want = [
  'w4-status.json',
  'e2e-js-scan.json', 'e2e-js-heal-dry.json',
  'e2e-py-scan.json', 'e2e-py-heal-dry.json',
  'e2e-shadow.json', 'e2e-pr-dry.json'
];
const present = {};
for (const f of want) {
...
const verifyFiles = fs.readdirSync(rptDir).filter(x => /^verify-.*\.json$/.test(x)).sort().reverse();
...
fs.writeFileSync(path.join(rptDir, 'w4-qa-summary.json'), JSON.stringify({ ok: true, present, verify, ts: now }, null, 2));
