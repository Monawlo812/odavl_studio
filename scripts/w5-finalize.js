...
function read(p){ try{ return JSON.parse(readFileSync(p,"utf-8")); }catch{ return undefined; } }
const artifacts = {
...
lines.push(`- Planner Tokens v2 sample: ${tokensV2 !== null ? "✅ ("+tokensV2+")" : "❌"}`);
if (artifacts.go)   lines.push(`- Go: lint=${artifacts.go.lintWarnings ?? "?"}, tests=${artifacts.go.testFailures ?? "?"}, vulns=${artifacts.go.vulns ?? "?"}`);
if (artifacts.java) lines.push(`- Java: lint=${artifacts.java.lintWarnings ?? "?"}, tests=${artifacts.java.testFailures ?? "?"}`);
if (artifacts.ablation) lines.push(`- Planner ablation: before=${artifacts.ablation.beforeOrder?.join(",")||"-"} → after=${artifacts.ablation.afterOrder?.join(",")||"-"}`);
lines.push("");
lines.push(`Overall OK: ${finalcheck.ok ? "✅" : "❌"}`);

mkdirSync("reports/w5",{ recursive:true });
writeFileSync("reports/w5/w5-finalcheck.json", JSON.stringify(finalcheck, null, 2));
