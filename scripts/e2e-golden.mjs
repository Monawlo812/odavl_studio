...
async function buildCli() {
  ...
  const buildResult = await run("pnpm --filter @odavl/cli run build", {
    timeout: 60000,
  });
  if (!buildResult.ok) {
...
async function runPr(results, savedFiles) {
  ...
  const prResult = await run(
    'node apps/cli/dist/index.js pr open --explain --dry-run --title "E2E Probe"',
  );
...
function printSummary(timestamp, savedFiles, steps) {
  ...
  console.error(
    `✨ Success rate: ${successful}/${total} steps (${Math.round((successful / total) * 100)}%)`,
  );
...
async function main() {
  ...
  if (existsSync(pyDist)) {
    // Scan
    pyScan = await run(
      `node -e "import('./${pyDist}').then(m=>m.scan().then(r=>console.log(JSON.stringify(r))))"`,
    );
    let pyScanObj = { ok: pyScan.ok, out: pyScan.stdout, err: pyScan.stderr };
    try {
      pyScanObj = { ...pyScanObj, parsed: JSON.parse(pyScan.stdout) };
    } catch {}
    saveResult("e2e-py-scan.json", pyScanObj);
    // Heal dry-run
    pyHeal = await run(
      `node -e "import('./${pyDist}').then(m=>m.heal({dryRun:true}).then(r=>console.log(JSON.stringify(r))))"`,
    );
    let pyHealObj = { ok: pyHeal.ok, out: pyHeal.stdout, err: pyHeal.stderr };
    try {
      pyHealObj = { ...pyHealObj, parsed: JSON.parse(pyHeal.stdout) };
    } catch {}
    saveResult("e2e-py-heal-dry.json", pyHealObj);
  } else {
    saveResult("e2e-py-scan.json", {
      ok: false,
      note: "python pack not built",
    });
    saveResult("e2e-py-heal-dry.json", {
      ok: false,
      note: "python pack not built",
    });
  }
  ...
  printSummary(timestamp, savedFiles, [
    buildResult,
    scanResult,
    healResult,
    shadowResult,
