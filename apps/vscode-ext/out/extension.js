...
function runInTerminal(cmd, name = 'ODAVL CI Doctor') {
            ...
            try {
                ...
                switch (msg?.type) {
                    ...
                    case 'heal': {
                        ...
                        if (msg.apply)
                            args.push('--apply');
                        const child = (0, child_process_1.spawn)(process.execPath, args, { cwd: root, shell: process.platform === 'win32' });
                        let out = '', err = '';
                        ...
                        child.on('close', () => {
                            try {
                                ...
                                panel.webview.postMessage({ type: 'healResult', data: json });
                            }
                            catch {
                                panel.webview.postMessage({ type: 'healResult', data: { pass: false, raw: out, stderr: err } });
                            }
                        });
                        break;
                    }
