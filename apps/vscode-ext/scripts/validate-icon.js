...
function validateIconFiles() {
  ...
  if (!checks.iconLightExists) {
    ...
    try {
      ...
      checks.iconLightValid =
        lightContent.includes("<svg") && lightContent.includes("</svg>");
  ...
  if (!checks.iconDarkExists) {
    ...
    try {
      ...
      checks.iconDarkValid =
        darkContent.includes("<svg") && darkContent.includes("</svg>");
...
function validateIcon() {
  ...
  try {
    ...
    Object.assign(
      results.checks,
      containerResult.checks,
      viewsResult.checks,
      commandsResult.checks,
      iconResult.checks,
    );
    results.errors.push(
    ...
    results.checks.activationEventsOk =
      activationEvents &&
    ...
    results.checks.validateScriptExists = !!(
      scripts && scripts["icon:validate"]
    );
...
function main() {
  ...
  if (results.ok) {
    ...
    console.log(
      `   - Container exists: ${results.checks.containerExists || false}`,
    );
    console.log(
      `   - Icon files present: ${results.checks.iconLightExists && results.checks.iconDarkExists}`,
    );
    console.log(
      `   - Commands configured: ${results.checks.commandsCount || 0}`,
    );
  } else {
