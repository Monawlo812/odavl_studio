...
app.post("/api/heal", async (req, res) => {
  ...
  const result = await runCLI(
    ["heal", "--recipe", "esm-hygiene", "--dry-run", "--max-files", "5"],
    `heal-${Date.now()}.json`,
  );
  res.json(result);
});
...
app.post("/api/magic", async (req, res) => {
  ...
  try {
    ...
    const scan = await runCLI(
      ["scan", "--json"],
      `magic-${timestamp}-scan.json`,
    );

    broadcastLog("Step 2/4: Healing...");
    const heal = await runCLI(
      ["heal", "--recipe", "esm-hygiene", "--apply", "--max-files", "3"],
      `magic-${timestamp}-heal.json`,
    );

    broadcastLog("Step 3/4: Shadow CI...");
    const shadow = await runCLI(
      ["shadow", "run", "--json"],
      `magic-${timestamp}-shadow.json`,
    );

    broadcastLog("Step 4/4: PR preparation...");
    const pr = await runCLI(
      ["pr", "open", "--explain", "--dry-run"],
      `magic-${timestamp}-pr.json`,
    );
    ...
    await fs.writeFile(
      path.join(reportsDir, `magic-${timestamp}-summary.json`),
      JSON.stringify(result, null, 2),
    );
    broadcastLog(
      `✅ Magic workflow complete! Results saved to magic-${timestamp}-summary.json`,
    );
...
app.get("/api/logs/stream", (req, res) => {
  ...
  res.write(
    'data: {"type":"connected","message":"Connected to ODAVL launcher logs"}\n\n',
  );
  sseClients.push(res);
