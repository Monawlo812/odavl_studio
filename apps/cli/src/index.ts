...
function findJsTsFiles(rootDir: string): string[] {
  ...
  function traverse(dir: string): void {
    try {
      ...
      console.warn(
        `CLI: Unable to read directory ${dir}:`,
...
function getLatestCIUrl(): string {
  try {
    const result = execSync(
      "gh run list --limit 1 --json url,headBranch,status,conclusion",
      { encoding: "utf8" },
    );
...
if (hasHelpFlag || cmd === "help") {
  ...
  console.log(
    "  heal           Fix code issues (--recipe remove-unused, --apply)",
  );
  console.log("  branch create  Create a new git branch");
  console.log(
    "  pr open        Open a pull request (--explain, --dry-run, --title)",
  );
  console.log(
    "  shadow run     Trigger CI workflow (--ref <branch>, --wait, --dry-run)",
  );
  console.log(
    "  shadow status  Check CI workflow status (--ref <branch>, --watch)",
  );
  console.log(
    "  status         Show current project status (branch, governor, telemetry, CI)",
  );
  console.log(
    "  governor explain  Show current governor status for PR and shadow operations",
  );
  console.log(
    "  report telemetry summary  Show usage analytics (--since 24h|<ISO>)",
  );
  console.log(
    "  undo last      Restore files from last heal --apply (--show to preview)",
  );
  console.log(
...
if (cmd === "scan") {
  ...
  if (args[i] === "--wf" && i + 1 < args.length) { wfName = args[i + 1]; }
  else if (args[i] === "--input" && i + 1 < args.length) { inputArg = args[i + 1]; }
  }
  if (!wfName) {
  ...
  if (typeof runWorkflow === "function") {
    ...
    process.stderr.write(
      "orchestrator not available — build/install @odavl/orchestrator\n",
    );
  }
  ...
  try {
    ...
    const addValidators = (result: any) => {
      if (validate) {
        // Run type-check
        const typecheck =
          runCmd("pnpm", ["-w", "-r", "run", "type-check"]) ||
          runCmd("pnpm", ["exec", "tsc", "--noEmit"]);
        // Run lint
    ...
    try {
      ...
      if (maxLinesMatch)
        maxLinesPerPatch = parseInt(maxLinesMatch[1], 10) || maxLinesPerPatch;
      if (maxFilesMatch)
        maxFilesTouched = parseInt(maxFilesMatch[1], 10) || maxFilesTouched;
    } catch {
    ...
    const chunkPatches = (
      patches: any[],
      budget: { maxLinesPerPatch: number; maxFilesTouched: number },
    ): any[][] => {
      const chunks: any[][] = [];
    ...
    const executeEsmHygiene = async () => {
      ...
      const filteredPatches = patchSet.patches.filter(
        (p: any) => !isProtectedPath(p.file),
      );
      ...
      const totalStats = {
        ...
        lines: filteredPatches.reduce(
          (sum, patch) => sum + estimatePatchLines(patch),
          0,
        ),
      };
    ...
    const processEsmHygienePatch = (patch: any, undoSnapshot: any): void => {
      try {
        ...
        console.warn(
          `CLI: Unable to apply ESM hygiene to ${patch.file}:`,
          error instanceof Error ? error.message : String(error),
        );
      }
    };

    const applyEsmHygieneChunks = async (
      chunks: any[],
      chunksMetadata: any[],
    ) => {
      let appliedChunk = 0;
      ...
      const result = addValidators({
        ...
        stats: {
          ...
          lines:
            appliedChunk > 0
              ? chunks[0].reduce(
                  (sum: number, patch: any) => sum + estimatePatchLines(patch),
                  0,
                )
              : 0,
        },
        notes: normalizeNotes(
          chunks,
          appliedChunk,
          pendingChunks > 0 ? pendingChunks : undefined,
        ),
      });
      console.log(stableJson(result));
    };
    ...
    const applyDependencyUpgrades = async (changes: any[]) => {
      ...
      for (const change of changes) {
        try {
          ...
          console.warn(
            `CLI: Unable to update dependencies in ${change.path}:`,
            error instanceof Error ? error.message : String(error),
          );
        }
      }
  ...
  try {
    ...
    gitCheck.stdout?.on("data", (data) => {
      gitStdout += data.toString();
    });
    gitCheck.stderr?.on("data", (data) => {
    ...
    gitCheck.on("close", (code) => {
      ...
      const branchCreate = spawn("git", ["checkout", "-b", branchName], {
        stdio: "pipe",
        shell: true,
      });
      let branchStdout = "";
      ...
      branchCreate.stdout?.on("data", (data) => {
        branchStdout += data.toString();
      });
      branchCreate.stderr?.on("data", (data) => {
  ...
  try {
    ...
    if (!title) {
      ...
      title =
        titleResult.status === 0 ? titleResult.stdout.trim() : `ODAVL: ${head}`;
    }
    ...
    if (explain) {
      ...
      const eslintDelta =
        isFinite(current.eslint) && isFinite(baseline.eslint)
          ? current.eslint - baseline.eslint
          : "n/a";
      const typesDelta =
        isFinite(current.typeErrors) && isFinite(baseline.typeErrors)
          ? current.typeErrors - baseline.typeErrors
          : "n/a";
      ...
      const workflowRunResult = run(
        `gh run list --branch ${head} --limit 1 --json url -q '.[0].url'`,
      );
      const workflowUrl =
        workflowRunResult.status === 0 && workflowRunResult.stdout.trim()
...
Evidence:
  ...
  let ref =
    refIndex !== -1 && refIndex + 1 < args.length ? args[refIndex + 1] : "";
  ...
  try {
    ...
    const upstreamResult = run(
      `git rev-parse --symbolic-full-name --abbrev-ref --quiet @{u}`,
    );
    ...
    let runResult = run(
      `gh run list --branch ${ref} --limit 1 --json databaseId,url,status,conclusion,headBranch,headSha,displayTitle -q .[0]`,
    );
  ...
  function fetchLatestRun(targetRef: string): any {
    try {
      const result = execSync(
        `gh run list --limit 1 --json url,headBranch,status,conclusion,workflowName,createdAt,updatedAt,headSha --branch ${targetRef}`,
        { encoding: "utf8" },
      );
      const runs = JSON.parse(result);
  ...
  cmd === "report" &&
  process.argv[3] === "telemetry" &&
  process.argv[4] === "summary"
) {
  try {
  ...
  if (hasPy) {
    ...
    sh("python", ["--version"]);
    pySignals = {
      ...readJSON("^python/scan-.*\\.json$"),
      ruff: ruffV.ok ? true : undefined,
    };
  }
  result.packs = {};
  ...
  if (gov.ok && gov.out) {
    try {
      result.governor = JSON.parse(gov.out);
    } catch {}
  }
  // Telemetry
  let mode =
    process.env.ODAVL_TELEMETRY ||
    (typeof readTelemetryMode === "function" ? readTelemetryMode() : "off") ||
    "off";
  ...
  if (pr.ok && pr.out) {
    try {
      ...
      result.ci.openPRs = Array.isArray(arr) ? arr.length : undefined;
      result.ci.top = (arr || []).slice(0, 3).map((x: any) => x.headRefName);
    } catch {
      result.ci.note += "pr-parse-fail; ";
    }
  } else {
    result.ci.note += "gh-missing; ";
  }
  let runs = sh("gh", [
    "run",
    "list",
    ...
    "5",
    "--json",
    "status,conclusion,htmlUrl,createdAt",
  ]);
  if (runs.ok && runs.out) {
    try {
      result.ci.lastRuns = JSON.parse(runs.out)
        .slice(0, 5)
        .map((r: any) => ({
          status: r.status,
  ...
  console.log(JSON.stringify(result));
  console.error(
    `ODAVL status: ${result.branch} | packs[js:${hasJs}] [py:${hasPy}] | ci:${result.ci.openPRs || 0} PRs`,
  );
} else if (cmd === "abort") {
  try {
    const workspaceRoot = path.resolve(__dirname, "../../..");
    ...
    try {
      const runsResult = spawnSync(
        ...
        [
          ...
          "in_progress",
          "--json",
          "databaseId,headBranch,displayTitle,url",
          "--branch",
          currentBranch,
        ],
        {
    ...
    for (const run of runs) {
      try {
        const cancelResult = spawnSync(
          "gh",
          ["run", "cancel", run.databaseId.toString()],
          {
            cwd: workspaceRoot,
            shell: true,
          },
