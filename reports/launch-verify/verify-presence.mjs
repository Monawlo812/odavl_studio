...
const checks = [
  ...
  // L-2: CLI publish prep
  {
    ...
    type: "json",
    keys: ["bin.odavl", "files", "engines.node", "scripts.prepublishOnly"],
  },
  ...
  // L-3: VS Code marketplace prep
  {
    ...
    type: "json",
    keys: ["displayName", "categories", "galleryBanner", "icon"],
  },
  ...
  {
    step: "L-4",
    file: "infra/github-app/manifest.json",
    type: "json",
    keys: ["default_permissions"],
  },
  ...
  try {
      ...
      try {
        ...
        if (missingKeys.length > 0) {
          return {
            ...check,
            exists: true,
            notes: `Missing keys: ${missingKeys.join(", ")}`,
          };
        }
        ...
        return {
          ...check,
          exists: true,
          notes: `Invalid JSON: ${err.message}`,
        };
      }
    }
    ...
    if (check.type === "content") {
      ...
      return {
        ...
        exists: true,
        notes: found
          ? `Pattern '${check.pattern}' found`
          : `Pattern '${check.pattern}' not found`,
      };
    }
...
for (const result of results) {
  const status =
    result.exists &&
    !result.notes.includes("Missing") &&
    !result.notes.includes("not found")
      ? "✅"
      : "❌";
  mdLines.push(
    `| ${result.step} | ${result.file} | ${status} | ${result.notes} |`,
  );
}

writeFileSync("reports/launch-verify/presence.md", mdLines.join("\n"));

console.log(`Verification complete. Checked ${results.length} items.`);
console.log(
  `Results: ${results.filter((r) => r.exists).length} exist, ${results.filter((r) => !r.exists).length} missing`,
);
